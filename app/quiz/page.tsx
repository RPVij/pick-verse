      ...
      <div className="w-full max-w-lg space-y-6 animate-pulse">
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            <div className="h-3 w-28 bg-gray-200 dark:bg-slate-700 rounded-full" />
            <div className="h-3 w-8 bg-gray-200 dark:bg-slate-700 rounded-full" />
          </div>
          <div className="h-1.5 w-full bg-gray-200 dark:bg-slate-700 rounded-full" />
        </div>

        {/* Question card skeleton */}
        <div className="rounded-3xl p-5 sm:p-8 border bg-white border-gray-100 shadow-sm dark:bg-[#1c1928] dark:border-[#2e2b40] space-y-5">
          ...
          <div className="space-y-2">
            <div className="h-4 bg-gray-100 dark:bg-slate-700 rounded-full w-full" />
            <div className="h-4 bg-gray-100 dark:bg-slate-700 rounded-full w-3/4" />
          </div>
          <div className="space-y-3 pt-1">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-14 bg-gray-50 dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700" />
            ))}
          </div>
        </div>

        {/* Button skeleton */}
        <div className="h-14 bg-indigo-100 dark:bg-indigo-900/30 rounded-2xl" />
      </div>
    </PageShell>
  );
}
...
export default function QuizPage() {
  ...
  async function handleNext() {
    ...
    if (isLast) {
      ...
      // Persist full quiz data so result page can show the breakdown
      try {
  ...
  return (
    <PageShell>
      <div className="w-full max-w-lg space-y-6">
        ...
        <div className={`rounded-3xl p-5 sm:p-8 space-y-5 border transition-opacity duration-200
        ...
        >
          ...
          <h2 className="text-lg sm:text-xl font-semibold leading-snug text-gray-900 dark:text-slate-100">
            {question.question}
          </h2>
